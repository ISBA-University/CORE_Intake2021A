...
knitr::include_url("https://www.tagesschau.de/wirtschaft/konjunktur/ein-jahr-wumms-konjunkturprogramm-corona-101.html",height = "470px")
```
]
...
```{tikz ,echo=FALSE, message=FALSE}
  	\definecolor{ISBAblu}{HTML}{232461}
	\definecolor{ISBAred}{HTML}{d84116}
	\definecolor{dgreen}{HTML}{124429}
...
{\sffamily
	\begin{tikzpicture}[scale=.6,
		...
		font=\normalsize,node distance=15em and 10em},
		pole/.style = {node,font=\bfseries,rectangle,rounded corners,very  thick,minimum height=4.5em,minimum width=9em,	
		draw=ISBAblu!60,top color=white, bottom color=mblu!50,
		drop shadow,scale=.7},
		market/.style = {node, ellipse, minimum height=4em,minimum width=9em, 
		draw=ISBAblu!60,top color=white, bottom color=mblu!50,
		drop shadow,scale=.6},
%		plainode/.style = {font=\bfseries,node,fill=none,scale=.7},
		Fin1/.style={ellipse, minimum height=5em,minimum width=9em, 
			draw=nblu,top color=white, bottom color=mblu!50,
			drop shadow,scale=.5},
		dev/.style = {node, ellipse, minimum height=2em,minimum width=5em, 
			draw=nblu,top color=white, bottom color=mblu!25,
			scale=.4},
		Fin2/.style = {node, fill=none,anchor=north,scale=.5},
		plainode/.style = {node,fill=none,scale=.4,font=\scriptsize},% arbre rouge noir, noeud rouge
		line/.style={draw=ISBAblu!50, line width=.6em},	
		arr/.style={draw=ISBAblu!50, -stealth, line width=.6em}			
		]
   \usetikzlibrary{calc,backgrounds,shadows,shadows.blur,shadings,shapes,shapes.geometric};
		\coordinate (P1) at (-4.5,-.5);
		\coordinate (P2) at (4.5,-.5);
		\coordinate (P3) at (1,-.5);
  	\node[pole] (p1) at (P1) {Unternehmen};	
		\node[pole] (p2) at (P2) {Private\\Haushalte};
%		\node[pole] (p3) at (P3) {Staat};
  	\coordinate (M1) at (0,3.5);
		\coordinate (M2) at (0,-3.5);
		\node[market] (Goods) at (M1) {Gütermärkte};
		\node[market] (Factors) at (M2) {Faktormärkte};
		\coordinate (Fin) at (-1.5,2);
%		\coordinate (Finlab) at (-1.8,1.9);
%		\coordinate (Dev) at (-1.55,1.35);	
   		\node[market] at (Fin) {Finanzmärkte};
%	   	\node[plainode,anchor=north] at (Finlab) {Finanzmärkte};
%  		\node[dev] at (Dev) {Devisen-\\markt};
		%Hilfskoordinaten
		\coordinate (M1o) at (-.5,3.75);
		\coordinate (M1u) at (-.9,3.25);
		\coordinate (p1ne) at (-3.75,0.2);
		\coordinate (p1nw) at (-5.2,0.2);
		\coordinate (p2nw) at (3.75,0);
		\coordinate (p2ne) at (5.3,0);
		\coordinate (p1ene) at (-3.5,-.4);
		\coordinate (p1ese) at (-3.5,-0.8);
		\coordinate (Finsw) at (-2.1,1.6);
		\coordinate (Finse) at (-1,1.6);
		\coordinate (Fins) at (-1.4,1.55);

		\begin{scope}[on background layer]	
			%Gütermärkte
			\path [line,rounded corners] (p2ne)  |-  (M1);
			\node[plainode] at (2.7,3.5) {Konsumausgaben (C)};
			\path [arr,rounded corners] (M1o) -| (p1nw);
			\node[plainode] at (-2.7,3.75) {Erlöse};	
			%Faktormärkte				
			\path[line,rounded corners] (p1.south)  |- (M2);
			\node[plainode] at (-2.7,-3.5) {Faktorentgelte};
			\path[arr,rounded corners] (M2) -| (p2.south);
			\node[plainode] at (2.78,-3.5) {Einkommen ($Y^b$)};
			%Finanzmärkte
			\path[line,rounded corners] (p2nw)  |- (Fin);
			\node[plainode] at (1.5,2) {Ersparnis Haushalte ($S^H$)};
			\path[arr,rounded corners]  (Fin)  -| (p1ne); 
			\node[plainode,rotate=90] at (-3.75,1.3) {Finanzierung};
		
			\path[arr,rounded corners]  (p1.north)  |- (M1u); 
			\node[plainode] at (-3,3.25) {(Brutto-)Investitionen ($I^b$)};
			\path[arr,rounded corners]  (p1ene)  -| (Finsw); 
			\node[plainode,rotate=90] at (-2.1,.4) {Abschreibungen ($\delta$)};
			\path[arr,rounded corners]  (p1ese)  -| (Fins);
			\node[plainode] at (-1.9,-.8) {Ersparnis};
			\node[plainode,rotate=90] at (-1.4,.25) {der Unternehmen ($S^U$)};
		\end{scope}	
		%	<4-> \node[draw, rounded corners] (text) at (0,0)  {
		%	   	\begin{minipage}{45mm}
	\end{tikzpicture}
}
```
...
.quellePan[Quelle: [Baldwin (2020).](https://voxeu.org/article/how-should-we-think-about-containing-covid-19-economic-crisis)]
]
]
...
```{tikz ,echo=FALSE, message=FALSE}
{\sffamily
	\begin{tikzpicture}[scale=0.25,text1/.style={font=\tiny}]
      ...
      \coordinate (O) at (0,0);
		...
		%Koordinatensystem
			...
			\node[text1, below,text centered,align=left] at (10,0) {Gesamtwirtschaftliches Angebot bzw. Einkommen (Y)};
			\draw[-stealth, thick] (0,0) -- (0,15) node {};
			\node[text1, above, rotate=90] at (0,7.5) {Gesamtwirtschaftliche Nachfrage (AD)}; 
    %Funktion
      ...
      \draw[name=Kapaz] (10,0) -- (10,14) node[text1,above,text centered] {Kapazitätsgrenze ($Y^V$)};
		\end{tikzpicture}
}
```
...
{\sffamily
	\begin{tikzpicture}[scale=0.3]
			...
			\draw[-stealth, thick] (0,0) -- (20,0) node[below, text width=1cm,text height =6pt, text centered] {Einkommen\\[-1.5mm] (Y)};
			\draw[-stealth, thick] (0,0) -- (0,15) node[left,text width=1.4cm,text height =6pt,text centered] {Konsum\\[-1.5mm] (C)};			
...
```{r ,echo=FALSE, message=FALSE,fig.align='center',outwidth='100%'}
knitr::include_url('https://www.core-econ.org/the-economy/book/images/web/figure-14-03.jpg',height='550px')
```
...
%	\draw[name=Kapaz] (10,0) -- (10,14) node[text1,above,text centered] {Kapazitätsgrenze (N)};
	\end{tikzpicture}
```
]
...
```{r ,echo=FALSE, message=FALSE,fig.align='center',outwidth='100%'}
knitr::include_url('https://www.core-econ.org/the-economy/book/images/web/figure-14-mcq-02.jpg',height = '400px')
```

.small[
**Wirkungskette:** zusätzliche Investitionsausgaben $\Rightarrow$ Zunahme AD $\Rightarrow$ Zunahme von Produktion und Einkommen $\Rightarrow$ weitere Zunahme von Nachfrage und Einkommen $\Rightarrow$ Neues Gleichgewicht
]

]
.panel[.panel-name[Disinvestition]
```{r ,echo=FALSE, message=FALSE,fig.align='center',outwidth='100%'}
knitr::include_url('https://www.core-econ.org/the-economy/book/images/web/figure-14-05-i.jpg',height = '400px')
```

.small[
**Wirkungskette:** Kürzung von Investitionsausgaben $\Rightarrow$ Rückgang AD $\Rightarrow$ Rückgang von Produktion und Einkommen $\Rightarrow$ weitere Rückgänge von Nachfrage und Einkommen $\Rightarrow$ Neues Gleichgewicht
]
]
]
...
```{r ,echo=FALSE, message=FALSE,fig.align='center',outwidth='100%'}
knitr::include_url('https://www.core-econ.org/the-economy/book/images/web/figure-14-06-d.jpg',height = '550px')
```
...
```{r,echo=FALSE,fig.align='center',out.width='100%'}
knitr::include_url('https://www.core-econ.org/the-economy/book/images/web/figure-14-09-e.jpg',height = '550px')
```
...
```{r,echo=FALSE,fig.align='center',out.width='100%'}
knitr::include_url('https://www.core-econ.org/the-economy/book/images/web/figure-14-10-c-c.jpg
',height = '500px')
```
...
```{r,echo=FALSE,fig.align='center',out.width='100%'}
knitr::include_url('https://www.core-econ.org/the-economy/book/images/web/figure-14-11-a-c.jpg
',height = '500px')
```
...
```{r,echo=FALSE,fig.align='center',out.width='100%'}
knitr::include_url('https://www.core-econ.org/the-economy/book/images/web/figure-14-11-b-c.jpg',height = '500px')
```
...
knitr::include_url('https://i2.wp.com/www.fiwi1.uni-freiburg.de/forschungszentrum-generationenvertraege/wp-content/uploads/sites/2/2020/03/fiskn1.png?ssl=1',height = '500px')
```
...
```{r,echo=FALSE,fig.align='center',out.width='100%'}
knitr::include_url('https://www.core-econ.org/the-economy/book/images/web/figure-14-16.jpg',height = '550px')
```
